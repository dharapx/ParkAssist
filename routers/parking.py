...
from baseConfig.models import ParkingLotDetails, ParkingLotUpdateRequest, ParkingAvailabilityResponse, UpdateAvailabilityRequest, UpdateAvailabilityResponse
...
def update_availability(req: UpdateAvailabilityRequest, current_user=Depends(get_current_user)):
    ...
    response = fetch_one(table="parking_lot", filters={"code": req.parking_lot_code})
    if response.get("error"):
        raise HTTPException(status_code=500, detail=response["error"])
